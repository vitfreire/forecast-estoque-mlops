...
def load_model_from_registry():
    """
    Carrega o modelo do MLflow Model Registry de forma genérica (pyfunc),
    evitando erro de flavor (lightgbm/xgboost/sklearn).
    Prioridade:
      1) stage Production (se existir)
      2) stage Staging (se existir)
      3) latest version
    """
    registered_name = os.getenv("MLFLOW_REGISTERED_MODEL_NAME", "").strip()
    if not registered_name:
        raise RuntimeError("MLFLOW_REGISTERED_MODEL_NAME não definido.")
    ...
    if prod:
        v = prod[0].version
        model_uri = f"models:/{registered_name}/{v}"
        return mlflow.pyfunc.load_model(model_uri)

    # tenta Staging
    stag = client.get_latest_versions(registered_name, stages=["Staging"])
    if stag:
        v = stag[0].version
        model_uri = f"models:/{registered_name}/{v}"
        return mlflow.pyfunc.load_model(model_uri)

    # fallback: última versão (sem stage)
    latest = client.get_latest_versions(registered_name)
    if not latest:
        raise RuntimeError("Nenhuma versão encontrada no Model Registry.")
    v = latest[0].version
    model_uri = f"models:/{registered_name}/{v}"
    return mlflow.pyfunc.load_model(model_uri)
